...
Page({
  ...
  tomember() {
    wx.navigateTo({
      url: '../weiworecord/weiworecord?title=领取记录',
    })
  },
  //跳转搜索页面
  search() {
    wx.navigateTo({
      url: `../search/search?city=${this.data.city}`,
    })
  },
  //地图方法
  //点击定位重新回到当前位置
  ...
  markertap: function (e) {
    ...
    if (this.data.active == 1) {
      ...
      if (markerId.MALLNAME) {
        ...
        //这里通过页面传参给详情页ID
        wx.navigateTo({
          url: `../markertap/markertap?id=${e.markerId}&static=${this.data.active}`,
        })
      }
    } else if (this.data.active == 3) {
      console.log(markerId)
  ...
  onmarkertap(e) {
    ...
    if (this.data.active == 1) {
      //这里通过页面传参给详情页ID
      wx.navigateTo({
        url: `../markertap/markertap?id=${e.detail}&static=${this.data.active}`,
      })
    }
  ...
  onLoad: function (options) {
    var cell = wx.getStorageSync('CELLPHONE')
    if (cell) {
      this.setData({
        havePhone: 1
      })
