// pages/sharing/accept/accept.js
var app = getApp()
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.q) {
      ...
      let cellPhone = this.gup('cellPhone', queryAll);
      let chnnel = this.gup('chnnel', queryAll);
      this.setData({
        cell: cellPhone,
        chnnel: chnnel
      })
      // if (chnnel == 'WeChatAccount') {
      //   wx.reLaunch({
      //     url: `../../wechatactivities/welfare/welfare?cell=${this.data.cell}&chnnel=WeChatAccount`,
      //   })
      // }
    } else {
      this.setData({
        cell: options.cellphone || options.cellPhone,
        chnnel: options.chnnel
      })
    }
    ...
    this.getBG(that.data.bgpic).then(function (locationData) {
      that.setData({
        bgpic: locationData
      })
    })
  ...
  todetail() {
    if (this.data.wxcell) {
      wx.navigateTo({
        url: '../../wechatactivities/wxdetails/wxdetails?cell=' + this.data.wxcell,
      })
    } else {
      wx.showModal({
        ...
        success: (res) => {
          if (res.cancel) {
            ...
            wx.reLaunch({
              url: `../../wechatactivities/wxlogin/wxlogin`,
            })
          }
        }
      })
